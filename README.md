...
### 🛠️ 便利な応用機能
*   **整形ペースト機能**: `|` または `| ` の直後に複数行テキストを貼り付けた場合に、Pugのpipe行へ自動整形します。VS Codeでは通常の貼り付け（`Ctrl+V`または`Cmd+V`）でこの条件に一致する場合のみ介入し、それ以外の貼り付けはエディタ標準の動作に任せます。Cursorではpaste API互換性の制限を避けるため、同じ整形ロジックを使うフォールバックコマンド（`Ctrl+Shift+V`または`Cmd+Shift+V`）を利用できます。
...
*   **整形ペースト機能**:
    - VS Codeでは、Pugファイル内の `|` または `| ` の直後に複数行テキストを通常貼り付けすると、各行を `|` 付きで挿入します。pipe直後以外では標準の貼り付け動作になります。
    - Cursorでは、`Ctrl+Shift+V`（Macの場合は`Cmd+Shift+V`）または右クリックメニューの「Paste with Pug Formatting」で同じ整形を実行できます。
    - 「Paste with Pug Pipe and br」コマンドでは、複数行の間に `br` を挿入する既存モードも利用できます。
